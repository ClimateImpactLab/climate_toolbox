...
from climate_toolbox.io.load import standardize_climate_data
...
def clim_data(lat, lon, time):
    ...
    ds = xr.Dataset({'tas': (['lat', 'lon', 'time'],  temp)},
                    coords={'lon': lon,
                            'lat': lat,
                            'time': time})
...
def test_clim_data(clim_data):
    ...
    assert not clim_data.tas.isnull().any()
...
def make_holes(clim_data, lat):
    ...
    tmp = clim_data['tas'].values
    array = np.random.randint(0, N, 1)
    tmp[array] = np.nan

    clim_data['tas'].values = tmp
...
def test_reindex_spatial_weights(clim_data, weights):
    ...
    assert ds.tas.shape == (len(ds['lon']), len(ds['time']))
...
def test_weighting(clim_data, weights):
    ...
    ds = _reindex_spatial_data_to_regions(clim_data, weights)
    assert not ds.tas.isnull().any()

    wtd = _aggregate_reindexed_data_to_regions(
        ds, 'tas', 'popwt', 'ISO', weights)

    assert not wtd.tas.isnull().any()

    wtd = _aggregate_reindexed_data_to_regions(
        ds, 'tas', 'areawt', 'ISO', weights)
...
def test_convert_kelvin_to_celsius(clim_data):
    ds = convert_kelvin_to_celsius(clim_data, 'tas')

    assert 'C' in ds.data_vars['tas'].units
...
def test_snyder_edd():
    ...
    assert res.edd.units == 'degreedays_281.15K'
    assert res.edd.sum().item(0) == 0.0


def test_snyder_gdd():
    ds_tasmax = xr.Dataset(
        data_vars={'tasmax': ('(latitude, longitude)', [280.4963, 280.7887], {'units': 'K'})},
        coords={'latitude': [-33.625, -33.375], 'longitude': [286.125, 286.375]})

    ds_tasmin = xr.Dataset(
        data_vars={'tasmin': ('(latitude, longitude)', [278.902, 278.23163], {'units': 'K'})},
        coords={'latitude': [-33.625, -33.375], 'longitude': [286.125, 286.375]})
    ...
    assert not res.gdd.units == 'degreedays_281.15K'
    assert res.gdd.units == 'degreedays_274.15-281.15K'
    assert res.gdd.sum().item(0) == pytest.approx(11, 0.1)
